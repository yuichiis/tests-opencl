...
int main(int argc, char ** argv)
{
    ...
    alpha = { 2 , 0 };
    std::complex<float> hostX[NWITEMS];
    for(int i = 0; i < NWITEMS; i++)
    {
        hostX[i] = { (float)i , 0 };             // 0           : 255
    }
    ...
    //                                NULL, NULL );
    std::complex<float> *pX = hostX;
    cl_mem bufX = clCreateBuffer( context,
                                    CL_MEM_READ_WRITE|CL_MEM_COPY_HOST_PTR,
                                    NWITEMS * sizeof(std::complex<float>),
                                    (void*)pX, NULL );
    ...
    clEnqueueReadBuffer(queue,
                        ...
                        0,                          // offset,
                        NWITEMS * sizeof(std::complex<float>), // size,
                        (void *)hostX,              // ptr,
    ...
    for(i=0; i < NWITEMS; i++) {
        printf("%d:(%6.2f,%6.2f), ", i, hostX[i].real(),hostX[i].imag());
    }
    printf("\n");
